...
pdf(here::here("figures", "distributions.pdf"))
for(v in pa_vars){
  p = joined %>%
    select(var = all_of(v), gender, age_cat) %>%
    ...
    labs(x = "Value", y = "Density", title = paste0("Distribution of ", v))
...
pdf(here::here("figures", "distributions_win.pdf"))
...
for(v in pa_vars){
  p = joined %>%
    ...
    filter(!is.na(age_cat)) %>%
    group_by(age_cat) %>%
    mutate(across(var, ~DescTools::Winsorize(.x, quantile(.x, probs = c(0, 0.99))))) %>%
    ggplot(aes(x = var, color = gender)) +
    geom_density() +
    facet_wrap(~age_cat, scales = "free_y") +
    theme(legend.position = "bottom") +
